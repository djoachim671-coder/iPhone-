...
function AppDashboard({onLogout}) {
  ...
  // ── Trade Analyst AI ──
  // Konvèti NENPÒT fòma imaj (HEIC, HEIF, PNG, JPG...) an JPEG atravè yon canvas.
  // Sa evite depann sou f.type, ki souvan vid oswa "image/heic" sou iPhone
  // — fòma API vizyon an pa aksepte dirèkteman.
  const handleAnalystFile = e => {
    ...
    reader.onload = ev => {
      ...
      img.onerror = () => {
        toast("❌ Format d'image non supporté par le navigateur", "error");
        setAnalystText("⚠️ Impossible de lire cette image. Essayez une CAPTURE D'ÉCRAN (screenshot) plutôt qu'une photo.");
      };
      img.src = dataUrl;
    };
