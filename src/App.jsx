...
function AppDashboard({onLogout}) {
  ...
  // ── Trade Analyst AI ──
  // Dekode fichye a dirèkteman ak createImageBitmap (pi fyab pou HEIC sou
  // Safari/iPhone pase yon <img> ki pa tache nan DOM — sa a ka rete tann pou
  // tout tan san onload NI onerror pou sèten fòma). Redimansyone imaj la si
  // li twò gwo (foto kamera iPhone yo souvan 12MP+) pou l pa depase limit
  // gwosè peyload Vercel la. Timeout ajoute pou pa janm rete "tann" san mesaj.
  const handleAnalystFile = async e => {
    ...
    const fail = (msg) => {
      toast("❌ " + msg, "error");
      setAnalystText(`⚠️ ${msg} Essayez une CAPTURE D'ÉCRAN plutôt qu'une photo, ou changez le format dans Réglages → Appareil photo → Formats → "Le plus compatible".`);
    };
    ...
    try {
      ...
      // Metòd 1 (preferab): createImageBitmap dirèkteman depi File la.
      if (window.createImageBitmap) {
        try {
          bitmap = await withTimeout(createImageBitmap(f), 8000);
        } catch (_) { bitmap = null; }
      }

      // Metòd 2 (fallback): FileReader → <img>, si Metòd 1 pa disponib/echwe.
      if (!bitmap) {
        ...
        bitmap = await withTimeout(new Promise((resolve, reject) => {
          const im = new Image();
          im.onload = () => resolve(im);
          im.onerror = () => reject(new Error("décodage échoué"));
          im.src = dataUrl;
        }), 8000);
      }

      // Redimansyone si twò gwo (evite depase limit peyload Vercel ~4.5MB)
      const MAX_DIM = 1600;
      ...
      if (!b64) { fail("Impossible de convertir cette image."); return; }

      setAnalystImgMime("image/jpeg");
      setAnalystImg(b64);
      setAnalystText("");
      toast("✅ Image chargée! Cliquez Analyser.", "success");
    } catch (err) {
      fail(err?.message === "timeout" ? "Délai dépassé lors du chargement de l'image." : "Impossible de lire cette image.");
    }
  };
