...
function AppDashboard({onLogout}) {
  ...
  const handleAnalystFile = e => {
    ...
    if(!f) { toast("⚠️ Aucun fichier détecté (sélection annulée?)","error"); return; }
    toast(`📎 Fichier reçu: ${f.name||"sans nom"} (${f.type||"type inconnu"}, ${Math.round(f.size/1024)}KB)`,"info");
    const mime = f.type || "";
    const supported = ["image/jpeg","image/jpg","image/png","image/gif","image/webp"];
    if (!supported.includes(mime)) {
      toast(`❌ Format "${mime||"inconnu"}" non supporté. Utilisez PNG ou JPG (pas HEIC).`,"error");
      setAnalystText(`⚠️ Format d'image non supporté: ${mime||"inconnu"}. L'IA accepte uniquement JPG, PNG, GIF ou WEBP. Si vous avez pris une photo avec l'appareil iPhone, changez le format en Réglages → Appareil photo → Formats → "Le plus compatible" (JPEG au lieu de HEIC), ou faites une CAPTURE D'ÉCRAN du chart à la place.`);
      return;
    }
  ...
  const analyzeChart = async () => {
    ...
    setAnalystLoading(true); setAnalystText("");
    try {
      ...
      if (!res.ok) {
        // Montre erè EGZAK ki soti nan API a
        const apiMsg = d?.error?.message || d?.message || JSON.stringify(d).slice(0,200);
        setAnalystText(`❌ Erreur API (HTTP ${res.status}): ${apiMsg}`);
        setAnalystLoading(false);
        return;
      }
      ...
      if (!text) {
        setAnalystText(`⚠️ Réponse vide de l'IA. Réponse brute: ${JSON.stringify(d).slice(0,300)}`);
      } else {
        setAnalystText(text);
